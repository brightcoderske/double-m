  ...
  return (
    <main className="admin-controls">
      ...
      <div className="register-toolbar page-actions">
        ...
        <button className="button dark" onClick={() => setDialog(true)}>
          ＋ Record payment
        </button>
      </div>
      {dialog && (
      ...
      <section className="dash-panel register-panel">
        <div className="panel-heading">
          <h2>Payment bands</h2>
          <span>{data.feeBands.length}</span>
        </div>
        <div className="payment-table-wrap">
          <table className="payment-table">
            <thead>
            ...
            <tbody>
              {data.feeBands.map((band: any) => (
                <tr key={band.id}>
                  <td>{band.payer_role}</td>
                  <td>{band.fee_name}</td>
                  <td>KES {Number(band.salary_min).toLocaleString()}</td>
